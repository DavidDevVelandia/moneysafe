<template>
  ...
  .item {
      ...
      width: 4rem; /* Ancho fijo para cada botón */
      height: 4rem; /* Altura fija para cada botón */
  }
  ...
  import { ShowCategories } from "../../data/categories/categories"; // Importa la función que muestra las categorías
  import CategoryComp from "./CategoryComp.vue"; // Importa el componente de categoría
  import { changeName } from "@/data/selectedComponent"; // Importa la función para cambiar el nombre de la categoría seleccionada
  
  const categories = ref(ShowCategories()); // Crea una referencia reactiva para las categorías
  ...
  function refreshCategories() {
      ...
      categories.value = ShowCategories().value; // Actualiza las categorías llamando a la función
  }
